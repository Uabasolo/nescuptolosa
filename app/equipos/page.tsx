...
export default async function EquiposPage() {
  ...
  const teamsByGroup = teamsData.teams.reduce((acc: Record<string, typeof teams>, team) => {
    ...
    <>
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="text-center">
            ...
            <p className="text-base sm:text-lg text-blue-100">
              {teams.length} equipos en {Object.keys(teamsByGroup).length} grupos - NESCUP 2026
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 sm:py-12 bg-gradient-to-b from-white to-gray-50">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {Object.entries(teamsByGroup).sort(([a], [b]) => a.localeCompare(b)).map(([groupId, groupTeams], idx) => {
            const colors = [
              { gradient: 'from-blue-700 to-blue-900', border: 'border-blue-300' },
              { gradient: 'from-amber-500 to-amber-600', border: 'border-amber-300' },
              { gradient: 'from-green-600 to-green-700', border: 'border-green-300' }
            ];
            const color = colors[idx % colors.length];
            
            return (
              <div key={groupId} className="mb-8 sm:mb-12">
                <div className="flex items-center justify-center mb-4 sm:mb-6">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 bg-gradient-to-br ${color.gradient} rounded-full flex items-center justify-center shadow`}>
                      <span className="text-xl font-bold text-white">{groupId}</span>
                    </div>
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Grupo {groupId}</h2>
                  </div>
                </div>
                
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{groupTeams.map((team) => (
